...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		...
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			p1 = input[tmp2 + j - 1] * horiz_operator[0][0]
				+ input[tmp2 + j] * horiz_operator[0][1]
				+ input[tmp2 + j + 1] * horiz_operator[0][2]
				+ input[tmp1 + j - 1] * horiz_operator[1][0]
				+ input[tmp1 + j] * horiz_operator[1][1]
				+ input[tmp1 + j + 1] * horiz_operator[1][2]
			 	+ input[tmp3 + j - 1] * horiz_operator[2][0]
				+ input[tmp3 + j] * horiz_operator[2][1]
				+ input[tmp3 + j + 1] * horiz_operator[2][2];
		
			p2 = 0;
	
			p2 = input[tmp2 + j - 1] * vert_operator[0][0]
				+ input[tmp2 + j] * vert_operator[0][1]
				+ input[tmp2 + j + 1] * vert_operator[0][2]
				+ input[tmp1 + j - 1] * vert_operator[1][0]
				+ input[tmp1 + j] * vert_operator[1][1]
				+ input[tmp1 + j + 1] * vert_operator[1][2]
			 	+ input[tmp3 + j - 1] * vert_operator[2][0]
				+ input[tmp3 + j] * vert_operator[2][1]
				+ input[tmp3 + j + 1] * vert_operator[2][2];
