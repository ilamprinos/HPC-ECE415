...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			p1 = input[(i -1)*SIZE + j -1] * horiz_operator[0][0]
				...
				+ input[(i +1)*SIZE + j +1] * horiz_operator[2][2];
		
			p2 = 0;
	
			p2 = input[(i -1)*SIZE + j -1] * vert_operator[0][0]
				...
				+ input[(i +1)*SIZE + j +1] * vert_operator[2][2];	
