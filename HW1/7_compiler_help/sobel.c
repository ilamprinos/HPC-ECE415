...
double sobel(unsigned char *input, unsigned char *output,unsigned char *golden)
{
	...
	register int i, j, tmp1, tmp2, tmp3, tmp4, tmp5;
	register unsigned int p;
	register int res,p1=0,p2=0;
	const int for_size = SIZE-1;
	struct timespec  tv1, tv2;
	FILE *f_in, *f_out, *f_golden;
	unsigned char root_lookup[65025];
	...
	for (i = 0; i < 65025; i++){
		root_lookup[i] = (unsigned char)(int)sqrt(i);
	}
	...
	for (i=1; i<for_size; i+=1) {
		...
		for (j=1; j<for_size; j+=1 ) {
			...
			p1 = - input[tmp2 + tmp4] + input[tmp2 + tmp5]
				- (input[tmp1 + tmp4] << 1) + (input[tmp1 + tmp5] << 1)
				- input[tmp3 + tmp4] + input[tmp3 + tmp5] ;
	
			p2 = input[tmp2 + tmp4] + (input[tmp2 + j] << 1)
				+ input[tmp2 + tmp5] - input[tmp3 + tmp4]
				- (input[tmp3 + j] << 1) - input[tmp3 + tmp5];	
	...
	for (i=1; i<for_size; i++) {
		...
		for ( j=1; j<for_size; j++ ) {
			t = (output[tmp1+j] - golden[tmp1+j])*(output[tmp1+j] - golden[tmp1+j]);
			PSNR += t;
		}
	}
